...
<div class="container my-5">
    ...
    <h1 class="my-5 text-center">Daftar Absensi Admin</h1>
    ...
    <select class="form-select" name="jenis_absen" id="jenis_absen">
        ...
        <option value="Masuk" @if($jenis_absen == 'Masuk') selected @endif>Masuk</option>
        <option value="Pulang" @if($jenis_absen == 'Pulang') selected @endif>Pulang</option>
    </select>
    ...
    <div class="container">
        {{-- keterangan user start --}}
        <div class="card mt-3">
            <div class="card-header">
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="card-title">Total User</h5>
                    <span class="badge badge-primary">{{ count($absences_arr) }} User</span>
                </div>
            </div>
            <div class="card-body">
                <ul class="list-group">
        ...
        {{-- hadir start --}}
        @foreach($absences_arr as $user => $absences)
            <div class="card mt-3">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="card-title">{{ $user }}</h5>
                        <span class="badge badge-primary">{{ count($absences) }} absences</span>
                    </div>
                </div>
                <div class="card-body">
                    <ul class="list-group">
                        @foreach($absences as $absence)
                            <li class="list-group-item">
                                <span 
                                @if($absence['status_absen'] == 'Tepat Waktu')
                                    class="badge badge-success"
                                @elseif($absence['status_absen'] == 'Terlambat')
                                    class="badge badge-danger"
                                @else
                                    class="badge badge-warning"
                                @endif
                                >{{ $absence['jenis_absen'] }}</span>
                                [{{ $absence['status_absen'] }}] [{{ $absence['waktu'] }}] [{{ $absence['posisi_absen'] }}]
                            </li>
                        @endforeach
                    </ul>
                </div>
        ...
        @endforeach
        {{-- hadir end --}}
        @foreach($absents_arr as $user => $absences)
            <div class="card mt-3">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="card-title">{{ $user }}</h5>
                        <span class="badge badge-primary">1 absent</span>
                    </div>
                </div>
                <div class="card-body">
                    <ul class="list-group">
                        ...
                        <li class="list-group-item">
                            <span class="badge badge-secondary">Izin</span>
                                [{{ $absence['waktu'] }}] [Keterangan: {{ $absence['reason_absen'] }}]
                        </li>
                        @endforeach
                    </ul>
                </div>
    ...
    <script type="text/javascript">
        ...
        document.getElementById('searchData').onclick = function () {
                ...
                window.location.href = "/absence/" + day + "/" + month + "/" + year + "/" + jenis_absen;
        };
        ...
        document.getElementById('prevDay').onclick = function () {
            ...
            window.location.href = "/absence/" + day + "/" + month + "/" + year + "/all";
        };
